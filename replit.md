...
- Knowledge base content (July 2026 import): 278 PDFs (~2 GB) bulk-imported from the user's Google Drive ZIP via `src/scripts/bulk-import-knowledge.ts` (resumable, dedups by sha256, sorts smallest-first). 276 ingested successfully (~21k chunks); 2 failed as corrupt `.crdownload` files. Training data: 500 real Q&A rows from the user's Excel live in `training_records` (sheet='excel_import') and `data/training-questions.json` was regenerated to match, so fresh deployments seed real data.
