...
function validateUpload(file: Express.Multer.File) {
  ...
  if (!allowedMimeTypes) return { error: "Unsupported file extension." } as const;
  if (!allowedMimeTypes.has(file.mimetype || "application/octet-stream")) {
    return { error: "Unsupported MIME type." } as const;
  }
  const maxBytes = Number(process.env.MAX_KNOWLEDGE_UPLOAD_BYTES ?? 50 * 1024 * 1024);
  if (file.size <= 0) return { error: "Uploaded file is empty." } as const;
  if (file.size > maxBytes) return { error: "Uploaded file exceeds the configured size limit." } as const;
  return { extension };
}

export async function storeKnowledgeUpload(file: Express.Multer.File, userId: string) {
  ...
  try {
    ...
    const document = await client.query<{ id: string }>(
      `INSERT INTO documents (title, created_by) VALUES ($1, $2) RETURNING id`,
    ...
    const job = await client.query<{ id: string }>(
      `INSERT INTO ingestion_jobs (document_version_id) VALUES ($1) RETURNING id`,
      [version.rows[0]!.id],
    );
    await client.query(
